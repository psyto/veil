...
export async function fetchSovereignIdentity(
  ...
  // Skip created_at (8 bytes) and authorities (5 * 32 bytes)
  // Scores start at offset 208
  const tradingScore = data.readUInt16LE(208);
  const civicScore = data.readUInt16LE(210);
  const developerScore = data.readUInt16LE(212);
  const infraScore = data.readUInt16LE(214);
  const creatorScore = data.readUInt16LE(216);
  const compositeScore = data.readUInt16LE(218);
