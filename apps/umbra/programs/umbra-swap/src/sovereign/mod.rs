...
impl SovereignIdentity {
    pub const SIZE: usize = 8 + 32 + 8 + (32 * 5) + (2 * 5) + 2 + 1 + 8 + 1;

    /// Tier field offset in the account data
    /// 8 (discriminator) + 32 (owner) + 8 (created_at) + 32*5 (authorities) + 2*5 (scores) + 2 (composite)
    pub const TIER_OFFSET: usize = 8 + 32 + 8 + (32 * 5) + (2 * 5) + 2;

    /// Composite score offset
    pub const COMPOSITE_SCORE_OFFSET: usize = 8 + 32 + 8 + (32 * 5) + (2 * 5);

    /// Trading score offset
    pub const TRADING_SCORE_OFFSET: usize = 8 + 32 + 8 + (32 * 5);
}
...
/// Convert SOVEREIGN tier to equivalent score for backward compatibility
pub fn sovereign_tier_to_fairscore(sovereign_tier: u8) -> u8 {
    match sovereign_tier {
        1 => 10,  // Bronze -> score ~10
        2 => 30,  // Silver -> score ~30
        3 => 50,  // Gold -> score ~50
        4 => 70,  // Platinum -> score ~70
        5 => 90,  // Diamond -> score ~90
        _ => 0,   // No identity -> score 0
    }
}
